(function() {
  ...
  function blinnPhongLighting(point, normal, viewPos, lightPos, mat, isEarth) {
    let ambient = 0.09;
    let specular = isEarth ? 0.12 : 0.38;  
    let shininess = 32;
  ...
  function loadSpheres() {
    const SPHERES_URL = "https://ncsucgclass.github.io/prog1/spheres.json";
    var httpReq = new XMLHttpRequest();
    httpReq.open("GET", SPHERES_URL, false); 
    httpReq.send(null);
    ...
    if ((httpReq.status !== 200) || (httpReq.readyState !== XMLHttpRequest.DONE)) {
      console.log("Unable to load spheres file.");
      return;
    }
    spheres = JSON.parse(httpReq.responseText);
  }
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        ...
        if (earthInt.hit && earthInt.t > 0) {
          ...
          let color = blinnPhongLighting(pt, n, eye, lightPos, mat, true);
